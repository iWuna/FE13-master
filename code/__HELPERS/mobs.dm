...
/proc/random_features()
	...
	if(!unihorn_list.len)
		init_sprite_accessory_subtypes(/datum/sprite_accessory/unihorn, unihorn_list)
	if(!ears_list.len)
	...
	return(list("mcolor" = pick("FFFFFF","7F7F7F", "7FFF7F", "7F7FFF", "FF7F7F", "7FFFFF", "FF7FFF", "FFFF7F"), "tail_lizard" = pick(tails_list_lizard), "tail_human" = "None", "wings" = "None", "snout" = pick(snouts_list), "horns" = pick(horns_list), "unihorn" = pick(unihorn_list), "ears" = "None", "frills" = pick(frills_list), "spines" = pick(spines_list), "body_markings" = pick(body_markings_list), "legs" = "Normal Legs"))
